...
Deno.test("test exporter()", function () {
...
Deno.test("test exporter(root = 0)", function () {
  const result = exporter(
    parse([
      { id: 1, parentId: 0 },
      { id: 2, parentId: 0 },
...
Deno.test("test exporter(root = fn)", function () {
  type Row = {
    id: number;
    parentId: number;
  };
...
Deno.test("test exporter(returnValue=nil)", function () {
  type Row = {
    id: number;
    parentId: number;
  };
